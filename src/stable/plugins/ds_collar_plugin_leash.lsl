...
integer find_leashpoint_link(){
    ...
    while (i <= n){
        ...
        if (nm == target && desc == target) return i;
        if (nm == target && name_match == 0) name_match = i;
        if (desc == target && desc_match == 0) desc_match = i;
        i = i + 1;
    }
    if (name_match != 0) return name_match;
    if (desc_match != 0) return desc_match;
    return LINK_ROOT;
}

//PATCH: Keep the cached leashpoint link synced with the actual prim.
integer ensure_leashpoint_link(){
    integer link = find_leashpoint_link();
    if (link <= 0) link = LINK_ROOT;
    if (link != g_leashpoint_link){
        if (g_psys_on && g_leashpoint_link != 0){
            llLinkParticleSystem(g_leashpoint_link, []);
            g_psys_on = FALSE;
        }
        g_leashpoint_link = link;
    }
    return g_leashpoint_link;
}
...
integer draw_leash_particles(key to){
    integer link = ensure_leashpoint_link();
    if (to == NULL_KEY){
        if (g_psys_on){ llLinkParticleSystem(link, []); g_psys_on = FALSE; }
        g_psys_target = NULL_KEY;
        return 0;
    }
    if (!g_psys_on || g_psys_target != to){
        g_psys_target = to;
        apply_ribbon_particles(link, to);
        g_psys_on = TRUE;
    }
    return 0;
}
